	...
	<div class="tiered-pricing-wrapper">
		...
		>
			...
			<div class="tiered-pricing-block tiered-pricing--active"
				...
				<div class="tiered-pricing-block__price">
					...
					<?php if ( $settings['show_discount_column'] ) : ?>
						...
						<?php
							// TRANSLATION LOGIC
							if ( $is_arabic ) {
								echo esc_html( sprintf( '(خصم %d%%)', round( $discountAmount, 2 ) ) );
							} else {
								echo esc_html( sprintf( '(%d%% off)', round( $discountAmount, 2 ) ) );
							}
						?>
						</span>
						<?php endif; ?>
				...
				<span class="tiered-pricing-block__quantity">
						<?php if ( 1 >= array_keys( $price_rules )[0] - $minimum || 'static' === $settings['quantity_type'] ) : ?>
							...
							<?php if ( $minimum > 1 ) : ?>
								<?php echo $is_arabic ? 'قطع' : esc_html( $settings['quantity_measurement_plural'] ); ?>
							<?php else : ?>
								<?php echo $is_arabic ? 'قطعة' : esc_html( $settings['quantity_measurement_singular'] ); ?>
							<?php endif; ?>
						<?php else : ?>
			...
			<?php while ( $iterator->valid() ) : ?>
				...
				// TRANSLATION LOGIC
				$quantity_measurement = $is_arabic ? 'قطع' : $settings['quantity_measurement_plural'];
				$quantity = $quantity . ' ' . $quantity_measurement;
